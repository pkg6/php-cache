...
namespace Pkg6\Cache;

use Pkg6\Cache\cache\CacheItem;
use Pkg6\Cache\cache\Driver;
use Pkg6\Cache\exception\InvalidArgumentException;
...
class CacheManager implements CacheItemPoolInterface
{
    ...
    public function connect(array $options, string $name = ''): Driver
    {
        ...
        $driver = __NAMESPACE__ . "\\cache\\driver\\" . $type;
        $handler = new $driver($options);
        if ($name) {
